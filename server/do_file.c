...
long int event_recv_file_ready(events *ev)
{
    ...
    // zlog_info(ser, "fille:%s f_size %ld toid %d", filename, f_size, toid);
    //加入数据库
    sprintf(ms->value,
...
void IN_recvfile(int cfd, int event, void *args)
{
    ...
    if (!recv_file(ev->fd, f_r[i].path, f_r[i].f_size))
    {
        ...
        sprintf(ev->js.value, "%d %s[%ld] error save", 0, f_r[i].path,
                f_r[i].f_size);
    }
    else
        sprintf(ev->js.value, "%d %s[%ld] success save", 1, f_r[i].path,
                f_r[i].f_size);
