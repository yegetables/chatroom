...
bool event_DEL_GROUP(info *ms)
{
    int group_id = -1;
    int user_id = -1;
    sscanf(ms->value, "select * from groups where  group_id=%d  and master_id=%d;", &group_id,
           &user_id);
    ...
    {
        sprintf(ms->value, "delete from groups where  group_id=%d  and master_id=%d;", group_id,
                user_id);
