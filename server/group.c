...
bool event_DEL_GROUP(info *ms)
{
    int group_id = -1;
    int user_id = -1;
    sscanf(ms->value, "select * from groups where  group_id=%d  and master_id=%d;", &group_id, &user_id);
    ...
    {
        sprintf(ms->value, "delete from groups where  group_id=%d  and master_id=%d;", group_id, user_id);
...
void del_relationship(int *id1, int *id2)
{
    ...
    {
        sprintf(p, "delete  from relationship where id_1=%d and id_2=%d ;", *id1, *id2);
    }
...
bool event_set_POWER_GROUP(info *ms)
{
    ...
    if (o1 == OWNER && o2 == PEOPLE && Authority == MANER)
    { //提权非2
        set_relationship(toid, ms->to, o2 | 2);
        strcpy(ms->value, "1");
        return true;
    }
    else if (o1 == OWNER && o2 == MANER && Authority == PEOPLE)
    { //降权与9
...
bool event_ADD_GROUP_APPLY(info *ms)
{
    ...
    int o1 = get_authority(ms->from, ms->to);
    int o2 = get_authority(toid, ms->to);
    if (o2 == NO_PEOPLE && Authority == PEOPLE)
    { //加人
        sprintf(ms->value,
