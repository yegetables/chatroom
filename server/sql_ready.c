...
reinit:;
    ...
    errornumber = 0;
reconnect:
    if (mysql_real_connect(conn, database_ip, database_user, database_passwd, NULL, 0, NULL, 0) == NULL)
    {
        errornumber++;
        zlog_error(ser,
                   "Failed to connect "
...
bool sql_init_table(MYSQL *sql_l)
{
    ...
    {
        memset(q, 0, sizeof(q));
        sprintf(q, "CREATE TABLE IF NOT EXISTS `user` ( "
    ...
    {
        memset(q, 0, sizeof(q));
        sprintf(q, "CREATE TABLE IF NOT EXISTS `relationship` ("
    ...
    {
        memset(q, 0, sizeof(q));
        sprintf(q, "CREATE TABLE IF NOT EXISTS `requests` ("
    ...
    {
        memset(q, 0, sizeof(q));
        sprintf(q, "CREATE TABLE  IF NOT EXISTS `groups` ("
